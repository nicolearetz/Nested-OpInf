...
from matrixSetup.BaseFood import BaseFood
...
class FoodTime(BaseFood):
    ...
    def set_data(self, snapshots, source, **kwargs):
        ...
        if snapshots is not None:
            ...
            self.training_snapshots = [
                snapshots[i][:, :: self.slicer] for i in range(self.nTrain)
            ]
            self.training_snapshots_original = [
                self.inverse_transform(snapshots[i][:, :: self.slicer])
                for i in range(self.nTrain)
            ]
            self.training_norms = kwargs.get(
                "training_norms", self.compute_training_norms()
            )
        ...
        if self.bool_rescale:
            raise NotImplementedError(
                "still need to bring up- and downscaling to arbitrary polynomial stting"
            )
    ...
    def compute_training_norms(self):
        ...
        for i in range(self.nTrain):
            training_norms[i] = self.fom.norm(
                self.training_snapshots[i],
                bool_summedTimestepNorm=self.bool_summedTimestepNorm,
