...
class BaseFood:
    ...
    def __init__(self, V, fom, **kwargs):
        ...
        self.V = V
        self.SPV = (
            self.V.T @ self.fom.SP @ self.V
        )  # self.inverse_transform(self.V).T @ self.fom.SP @ self.inverse_transform(self.V)
        ...
        self.SPW = (
            self.W.T @ self.fom.SP @ self.W
        )  # self.inverse_transform(self.W).T @ self.fom.SP @ self.inverse_transform(self.W)
    ...
    def set_data(self, snapshots, source, **kwargs):
        ...
        self.training_snapshots_original = [
            self.inverse_transform(snapshot) for snapshot in snapshots
        ]
        self.training_norms = kwargs.get(
            "training_norms", [self.fom.norm(snapshot) for snapshot in snapshots]
        )

        # keep the training projections as list such that we can distinguish between parameters
        self.training_proj_original = kwargs.get("training_proj", None)
        if self.training_proj_original is None:
            self.training_proj_original = [
                la.solve(self.SPV, self.V.T @ self.fom.SP @ self.training_snapshots[i])
                for i in range(self.nTrain)
            ]

        for i in range(self.nTrain):
            if len(self.training_proj_original[i].shape) < 2:
                self.training_proj_original[i] = np.reshape(
                    self.training_proj_original[i], (self.nRB, 1)
                )
        ...
        if self.bool_rescale:
            raise NotImplementedError(
                "still need to adjust up- and downscaling to arbitrary polynomial setting"
            )
        ...
        for j in range(self.nTrain):
            ...
            for i in range(self.nP):
                ...
                if isinstance(qTheta[i], np.ndarray):
                    data = np.kron(
                        qTheta[i],
                        polymat.dataMatrix_p(
                            proj, self.polyOrders[i], polynomial=self.polynomial
                        ),
                    )
                elif isinstance(qTheta[i], list):
                    data = [
                        (
                            polymat.dataMatrix_p(
                                proj, self.polyOrders[i], polynomial=self.polynomial
                    ...
                    data = qTheta[i] * polymat.dataMatrix_p(
                        proj, self.polyOrders[i], polynomial=self.polynomial
                    )

                # stack everything for this parameter together
                D = np.hstack([D, data])

            # stack all the parameters together
    ...
    def projection_error(self, indices):
        ...
        for j in range(self.nTrain):
            proj = la.solve(
                SPV, V.T @ self.fom.SP @ self.training_snapshots[j]
            )  # projection <len(indices)> x <no of timesteps>
            # proj = V.T @ self.training_snapshots[j]
            ortho = (
                self.training_snapshots[j] - self.V[:, indices] @ proj
            )  # orthogonal complement
            norms = [
                np.sqrt(ortho[:, i].T @ self.fom.SP @ ortho[:, i])
    ...
    def projection_norm(self, indices):
        ...
        for j in range(self.nTrain):
            ...
            norms = [
                np.sqrt(proj[:, i].T @ self.fom.SP @ proj[:, i])
    ...
    def get_data_matrix(self, indices=None, **kwargs):
        ...
        colIndices = polymat.rowIndices(
            indices=indices,
            nRB=self.nRB,
            polyOrders=self.polyOrders,
            affineOrders=self.affineOrders,
        )
    ...
    def blow_up(
        self, indices, A_sub, new_shape, indices_testspace=None, nRB=None, **kwargs
    ):
        """
    ...
    def blow_down(self, indices, big_matrix, indices_testspace=None, nRB=None):
        ...
        if big_matrix.shape[0] != self.get_shape(n=nRB)[0]:
            raise RuntimeError(
                "In BaseFood.blow_down: encountered incorrect dimensions, big_matrix.shape={}, nRB={}".format(
                    big_matrix.shape, nRB
                )
            )
    ...
    def get_residual(self, A_bk, indices=None, indices_testspace=None, **kwargs):
        ...
        indices_testspace = (
            [*range(A_bk.shape[1])] if indices_testspace is None else indices_testspace
        )
        indices = indices_testspace if indices is None else indices

        if len(indices_testspace) != A_bk.shape[1]:
            raise RuntimeWarning(
                "In BaseFood.get_residual: enountered invalid test space indices"
            )

        R_sub = self.get_rhs_matrix(
            indices=indices, R=self.R, indices_testspace=indices_testspace
        )
    ...
    def get_rows_by_polynomial_terms(self, indices, nRB):
        if len(indices) < nRB:
            raise NotImplementedError(
                "still need to think about how to actually do this"
            )
    ...
    def get_shape(self, n, m=None):
        ...
        nRows = sum(
            [
                self.affineOrders[i] * polymat.compute_nFEp(n, self.polyOrders[i])
                for i in range(self.nP)
            ]
        )
    ...
    def get_reduced_model(self, A_new, indices=None, indices_testspace=None):
        ...
        if A_new.shape[0] != self.get_shape(n)[0]:
            A_new = self.blow_down(
                indices=indices, indices_testspace=indices_testspace, big_matrix=A_new
            )
        ...
        Qq = polymat.get_queue(
            A=A_new, polyOrders=self.polyOrders, affineOrders=self.affineOrders, nRB=n
        )
        ...
        if self.bool_rescale:
            ...
            raise NotImplementedError(
                "still need to bring up- and downscaling to arbitrary polynomial setting"
            )
        ...
        rom = self.initialize_reduced_model(
            indices=indices, indices_testspace=indices_testspace
        )
    ...
    def reconstruction_error(
        ...
        rom,
        indices=None,
        bool_return_convergence=False,
        bool_early_return=True,
        **kwargs
    ):
        """
        ...
        if indices is None:
            print(
                "In BaseFood.reconstruction_error: it can really lead to big trouble if no indices are provided"
            )
        ...
        for j in range(self.nTrain):
            ...
            u_oi, bool_result_tmp = rom.solve(
                bool_return_convergence=True, **rom_arguments
            )
            ...
            if self.bool_relativeError:
                error[j] /= rom.norm(
                    self.training_proj_original[j][indices, :], **rom_arguments
                )  # relative error
    ...
    def rom_arguments(self, j, **kwargs):
        ...
        arguments = {"para": self.Xi_train[j, :]}
        return arguments
