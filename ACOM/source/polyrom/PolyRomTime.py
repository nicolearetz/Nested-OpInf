import numpy as np
from methods.helpers_polyMat import exp_p, kron_p, keptIndices_p, polynomial_exp_p

# from matrixSetup.helpers_matrices import *
from methods.timestepping import implicit_euler, explicit_euler, RK_midpoint
...
class PolyRomTime(PolyRom):
    ...
    def __init__(self, V, fom, **kwargs):
        ...
        self.u0 = la.solve(self.M, V.T @ fom.M @ self.transform(fom.u0))
        ...
        self.kept = [
            keptIndices_p(self.nRB, j) for j in range(max(self.polyOrders) + 1)
        ]
    ...
    def solve_linear(self, **kwargs):
        ...
        if self.polynomial is not None:
            raise RuntimeError(
                "Linear solve not implemented for arbitrary polynomial basis"
            )
        ...
        return implicit_euler(
            grid_t, A[:mRB, :nRB], self.u0[:nRB], M=self.M_WV[:mRB, :nRB], **new_kwargs
        )
    ...
    def solve(self, **kwargs):
        ...
        if self.bool_restrict:
            ...
            if self.polynomial is None:
                ...
                def fct_explicit(x):
                    ...
                    summands = [
                        assembled[i] @ exp_p(x, self.polyOrders[i], self.kept)
            ...
            if self.polynomial is not None:
                raise NotImplementedError(
                    "Non-monomial polynomial encountered without matrix size restriction."
                )

            def fct_explicit(x):
                summands = [
                    assembled[i] @ kron_p(x, self.polyOrders[i])
                    for i in explicit_indices
                ]
                # todo: since exp_p is a recursive function, we can save some time here
                return sum(summands)
        ...
        if kwargs.get("bool_explicit_euler", False):
            ...
            return explicit_euler(
                ...
                self.u0,
                M=self.M_WV,
                bool_return_convergence=bool_return_convergence,
    ...
    def solve_ivp(self, **kwargs):
        ...
        if self.polynomial is not None:
            raise NotImplementedError("Non-monomial polynomial encountered.")

        if self.bool_restrict:

            def fct_explicit(t, x):
                summands = [
                    assembled[i] @ exp_p(x, self.polyOrders[i], self.kept)
                    for i in explicit_indices
                ]
    ...
    def norm(self, u_rb, **kwargs):
        ...
        norm2 = np.array(
            [u_rb[:, i].T @ self.SP[:n, :n] @ u_rb[:, i] for i in range(u_rb.shape[1])]
        )
