...
class PolyRom:
    ...
    def __init__(self, V, fom, **kwargs):
        ...
        self.SP = V.T @ fom.SP @ V  # V_orig.T @ fom.SP @ V_orig  # inner product
        self.M = V.T @ fom.M @ V  # V_orig.T @ fom.M @ V_orig  # mass matrix
    ...
    def set_decomposition(self, Qq, affineOrders=None):
        ...
        if len(affineOrders) != self.nP:
            raise RuntimeError(
                "provided list of affine dimensions does not match number of polynomial terms"
            )

        if Qq.shape != (self.nP,):
            raise RuntimeError(
                "provided list of matrix-queues does not match number of polynomial terms"
            )

        for i in range(self.nP):
            if Qq[i].shape[0] != affineOrders[i]:
                raise RuntimeError(
                    "provided list of matrix-queues does not match affine dimension"
                )
