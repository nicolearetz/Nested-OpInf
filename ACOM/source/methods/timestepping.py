...
def explicit_euler(grid_t, A, x_init, **kwargs):
    ...
    for i in range(1, grid_t.shape[0]):
        ...
        # else:
        #     if bool_square:
        #         b = la.lu_solve(LU, rhs)
        #     else:
        #         b = methods.solvers.least_squares(M, rhs)
...
def RK_midpoint(grid_t, x_init, fct_explicit, **kwargs):
    ...
    for i in range(1, grid_t.shape[0]):
        ...
        if np.isnan(b).any() or np.isinf(b).any() or la.norm(b, ord=np.inf) > 1e+12:
        ...
        if not bool_eye:
            print("RK_midpoint but not eye", M.shape, M)
            temp = U.T @ b
            temp = temp.T / S
            b = Vt.T @ temp.T
