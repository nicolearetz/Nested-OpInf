...
def regularized_least_squares(
    ...
    extension_R=None,
    bool_rescale=True,
    bool_collect_condition_numbers=False,
):
    ...
    if len(weights.shape) != 1:
        if weights.shape[1] == 1:
            ...
            return regularized_least_squares_by_columns(
                D=D,
                R=R,
                weights=weights,
                scale=scale,
                ...
                bool_rescale=bool_rescale,
                bool_collect_condition_numbers=bool_collect_condition_numbers,
            )
    ...
    if np.max(weights) == np.inf:
        ...
        A_sub = least_squares(
            D_extended, R_extended, cutoff=cutoff, bool_rescale=bool_rescale
        )
        sol[remaining, :] = A_sub
    ...
    if bool_collect_condition_numbers:
        return sol, np.linalg.cond(D_extended)

    return sol


def regularized_least_squares_by_columns(
    D,
    R,
    weights,
    scale=1,
    ...
    if bool_collect_condition_numbers:
        conds = []
        if extension_R is not None:
            raise NotImplementedError(
                "condition number collection not implemented for R != None"
            )
        for n in range(R.shape[1]):
            sol[:, [n]], cond = regularized_least_squares(
                D=D,
                R=R[:, [n]],
                weights=weights[:, n],
                scale=scale,
        ...
        for n in range(R.shape[1]):
            if extension_R is None:
                sol[:, [n]] = regularized_least_squares(
                    D=D,
                    R=R[:, [n]],
                    weights=weights[:, n],
                    scale=scale,
...
def lstsq_truncSVD(
    D, R, cutoff=std_cutoff, bool_rescale=True, bool_relative_cutoff=False
):
...
def lstsq_tuncSVD_with_nullspace(
    D, R, cutoff=std_cutoff, bool_relative_cutoff=True, cutoff_subspace=None
):
...
def lstsq_mat(D, R):
    ...
    M = np.block([[np.eye(D.shape[0]), D], [D.T, np.zeros((D.shape[1], D.shape[1]))]])
